...
$result=mysql_query("SELECT * FROM `rank1` WHERE name='$name' ",$link);
...
if(mysql_num_rows($result)!=NULL) //看該編號是否填過了
	...
	{
		mysql_free_result($result);
		$result=mysql_query("INSERT INTO `rank1` ( `name` ,`score`) 
			VALUES ('$name', '$score')",$link);
		mysql_free_result($result);
		$result=mysql_query("SELECT * FROM `rank1` ORDER BY `score` DESC");
		echo "<script type='text/javascript'>";
		echo "alert('新增成功');";
		echo "history.back();";
